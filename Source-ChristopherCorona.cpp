...
ifstream readMode;
ofstream writeMode;
int modeNotePlacement[10]{ 0,13,26,39,52,65,78,91,104,117}; //This is where mode breaks are located in the mode.txt file
string readCmd[12];
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		{
			cmdIndex[0]= SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
				change[0] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST2)
		{
			cmdIndex[1] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[1] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST3)
		{
			cmdIndex[2] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[2] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST4)
		{
			cmdIndex[3] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[3] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST5)
		{
			cmdIndex[4] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[4] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST6)
		{
			cmdIndex[5] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[5] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST7)
		{
			cmdIndex[6] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[6] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST8)
		{
			cmdIndex[7] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[7] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST9)
		{
			cmdIndex[8] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[8] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST10)
		{
			cmdIndex[9] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[9] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST11)
		{
			cmdIndex[10] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[10] = true;
		}
		if (HIWORD(wParam) == CBN_SELCHANGE && LOWORD(wParam) == GEST12)
		{
			cmdIndex[11] = SendMessage((HWND)lParam, (UINT)CB_GETCURSEL,
				(WPARAM)0, (LPARAM)0);
			change[11] = true;
		}
		//////////////////////////////////////////////////////////////////////
		...
		case ID_BUTTON1: //Select Port Button 
		{
			...
			if (errorCheck == true && portSelected == false)
			{
				...
				portSelected = false;
				PlaySoundA("nolinkfound", NULL, SND_ASYNC); //change sound?

			}
			if (portSelected == false && errorCheck == false)
			{
				portSelected = true;
				PlaySoundA("linked", NULL, SND_ASYNC); //notify user
		...
		{	if (portSelected == false) SetDlgItemText(hwnd, HANDLE_LINK, L"no link"); //no sound ?
	...
	//if not then write mode from keyboardindex
		...
		case ID_BUTTON3: //Open Handle Buton
		{
			...
			if (portSelected == true) { PlaySoundA("Openhandleactivated", NULL, SND_ASYNC); }
			...
			if (portSelected == true) {
				...
				//Mario Mode
				if (SubModeIndex == 2) { Keymode.setMario(); SetDlgItemText(hwnd, HANDLE_STATUS, L"Mario Selected");  ShellExecute(0, 0, L"http://www.freemario.org", 0, 0, SW_SHOW);}
				//Media Mode 
				...
				ShowWindow(hwnd, SW_MINIMIZE); //Minmizes Window Once Selected
			...
			case 2: 
			{
				if (portSelected == false) 	SetDlgItemText(hwnd, HANDLE_STATUS, L"please link handle first");
					...
					case 5:
					{
						//variable declarations
						setupMouseEvent(mouse);
						double x, y, xBound, yBound, mouseSpeed = 110;
						char tilt = '_', lastCommand = '_';
						keyboard keyHex; keyHex.setArrowkeys();
